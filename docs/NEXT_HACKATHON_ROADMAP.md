...
next builds for Arbitrum Open House London, the MetaMask agent-wallet safety
lane, and AI-agent trust programs. The product should stay native-preflight
first: no bridges, no custody, no live trading, no hidden wallet automation.
...
1. Arbitrum Open House London.
   This is the verified active priority. The shortest defensible path is an
   Arbitrum-native proof-contract demo that reuses the existing 0G receipt and
   keeps the product claim narrow: native agent-wallet preflight before any
   signer or operator action.
2. MetaMask agent-wallet safety lane.
   Treat MetaMask Connect, Snaps transaction/signature insights, Smart Accounts
   Kit, Advanced Permissions, and delegation docs as the next integration lane
   unless a fresher public sponsor or application page is re-verified. Keep the
   story honest: no claimed partnership, no live Snap publication, no live
   settlement, and no wallet flow claim until the tested wrapper exists.
3. ETHGlobal New York 2026 and NANDA Hack.
   Treat these as the next distribution lanes for the same middleware: one
   wallet/EVM focused, one AI-agent trust focused.
...
- Arbitrum Open House is the active onchain proof-contract demo.
- MetaMask is the next permission-firewall integration lane.
- The shared asset is the same ZeroGuard receipt hash, first generated by
  native preflight, then read back from an Arbitrum deployment and later reused
  in wallet-agent integrations.
